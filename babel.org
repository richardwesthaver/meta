...
#+begin_src shell :var src=(org-sbe org-current-h1-title) :results output replace
  cd ~/comp/$src
...
* make-info-tbl                                                   :org:fs:vc:
:PROPERTIES:
:ID:       d5ba2f3d-fc2d-4db6-8bbd-7ca440ff0e8c
:END:
#+name: make-info-tbl
#+header: :var version="0.1.0"
#+header: :var name=(org-sbe org-current-h1-title)
...
#+begin_src emacs-lisp :results table replace
  (let* ((src (concat dir name))
         ...
         (rf (format "[[vc:comp/%s][%s:%s]]" name num rev))
  ...
  #+RESULTS: make-info-tbl
  |---------+----------------------------|
  | name    | [[https://compiler.company/docs/org][org]]                        |
  | version | 0.1.0                      |
  | vc      | [[https://vc.compiler.company/comp/org][41+:0f4d1a0415d5]]           |
  | updated | 3 hours ago                |
  | lines   | 13143 = λ:12268 #:46 _:829 |
  | langs   | (Html Org Svg)             |
  |---------+----------------------------|
...
:END:
#+name: ls-files
#+begin_src sh :results silent :var dir=(expand-file-name "~/comp") name=(org-sbe org-current-h1-title)
...
:END:
#+name: hg-rev
#+begin_src sh :var src=(org-sbe org-current-h1-title)
...
:END:
#+name: hg-id-num
#+begin_src shell :var src=(org-sbe org-current-h1-title)
cd ~/comp/$src && hg id -n
#+end_src
...
#+begin_src emacs-lisp :results silent :eval no-export
  ...
  (slime-eval '(in-package :std-user))
#+end_src
