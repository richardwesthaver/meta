...
* skel-show
#+name: skel-show
#+begin_src shell :dir "~/comp/core" :var key=":id" :results replace
  echo $(skel show $key)
#+end_src
...
#+begin_src emacs-lisp :var project="."
  (let* ((project (project-current nil project))
         ...
         (tags (org-sbe skel-show (key "':tags'") ''(dir root)))
...
| root        | ~/comp/org/meta/                                  |
| vc          | :hg [[https://vc.compiler.company/meta][vc.compiler.company/meta]] :rev [[https://vc.compiler.company/meta/rev/06698c6708de][06698c6708de:8+]] |
| tags        | (core lisp rust emacs c)                          |
| updated     | 3 days ago                                        |
| langs       | (org)                                             |
| lines       | 1768 :λ 1632 :# 3 :_ 133                          |
|-------------+---------------------------------------------------|
...
#+name: files
#+begin_src sh :results replace table :dir "." :colnames yes :hlines yes :post format-files-tbl(*this*)
