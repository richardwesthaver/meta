...
This library depends on the [[vc:comp/core][core]] being properly installed and the
=core/emacs= init system loaded in emacs. See [[vc:comp/home][home]] for details.
...
#+begin_src emacs-lisp :results table replace
  (let* ((src (concat dir name))
         (age (org-sbe "hg-log-age" ''(src name)))
         (rev (org-sbe "hg-rev" ''(src name)))
         (num (org-sbe "hg-id-num" ''(src name)))
         (cc1 (org-sbe "tokei-dir-lines" ''(src name)))
         (cc2 (org-sbe "tokei-dir-langs" ''(src name)))
  ...
  | version | 0.1.0                      |
  | vc      | [[https://vc.compiler.company/comp/org][44+:2b6f731f3684]]           |
  | updated | nil                        |
  | lines   | 14812 = λ:13917 #:45 _:850 |
  | langs   | (Html Org Svg)             |
  |---------+----------------------------|
