  ...
  (let* ((proj (project-current nil project))
    ...
    `((file modified)
      ...
      ,@(mapcar (lambda (x)
                  ...
                  (when-let ((attr (file-attributes x)))
  		  (let ((y (if (string= (file-name-base x) "readme")
  			       (let ((s (butlast (file-name-split x))))
  				 (apply 'file-name-concat "/" (append s (list "."))))
  			     x)))
                      (list (format "[[%s][%s]]"
                                    (format link-fmt (string-replace root "" y))
  				  (let ((fname (file-name-sans-extension (string-replace project "" y))))
  				    (if (string= fname ".") "index" fname)))
...
| file      | modified            |
|-----------+---------------------|
| [[file:babel.org][babel]]     | 2025-05-10 22:17:48 |
| [[file:.][index]]     | 2025-05-10 17:03:17 |
